    ...
    Update = function() {
        ...
        for (var i = 0; i < ds_list_size(GAME.all_foes); i++) {
            ...
            if (point_distance_3d(position.x, position.y, position.z, foe.position.x, foe.position.y, foe.position.z) <= radius) {
                ...
                OnHit(foe);
                Destroy();
                Particles.part_emitter_stone_debris.setRegion(matrix_build(foe.position.x, foe.position.y, foe.position.z, 0, 0, 0, 1, 1, 1), 16, 16, 16, spart_shape_cube, ps_distr_linear, false);
